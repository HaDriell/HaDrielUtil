...
public final class VulkanUtil {
    ...
    public static long setupDebugging(VkInstance instance, int flags, VkDebugReportCallbackEXT callback) {
        ...
        int err = EXTDebugReport.vkCreateDebugReportCallbackEXT(instance, debugCreateInfo, null, pCallback);
        long handle = pCallback.get(0);
        memFree(pCallback);
        debugCreateInfo.free();

        if(err != VK_SUCCESS)
            throw new AssertionError("Failed to setup Debugging: " + err);
    ...
    public static DeviceAndGraphicsQueueFamily createDeviceAndGetGraphicsQueueFamily(VkPhysicalDevice physicalDevice) {
        ...
        memFree(pDevice);
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to create device: " + err);
        }
    ...
    public static ColorFormatAndSpace getColorFormatAndSpace(VkPhysicalDevice physicalDevice, long surface) {
        IntBuffer pQueueFamilyPropertyCount = memAllocInt(1);
        vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, pQueueFamilyPropertyCount, null);
        int queueCount = pQueueFamilyPropertyCount.get(0);
        VkQueueFamilyProperties.Buffer queueProps = VkQueueFamilyProperties.calloc(queueCount);
        ...
        for (int i = 0; i < queueCount; i++) {
            ...
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to physical device surface support: " + (err));
            }
        }
        ...
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to query number of physical device surface formats: " + (err));
        }
        ...
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to query physical device surface formats: " + (err));
        }
    ...
    public static long createCommandPool(VkDevice device, int queueNodeIndex) {
        ...
        memFree(pCmdPool);
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to create command pool: " + (err));
        }
        return commandPool;
    }
    ...
    public static VkCommandBuffer createCommandBuffer(VkDevice device, long commandPool) {
        ...
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to allocate command buffer: " + (err));
        }
    ...
    private static Swapchain createSwapChain(VkDevice device, VkPhysicalDevice physicalDevice, long surface, long oldSwapChain, VkCommandBuffer commandBuffer, int newWidth,
        ...
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to get physical device surface capabilities: " + (err));
        }
        ...
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to get number of physical device surface presentation modes: " + (err));
        }
        ...
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to get physical device surface presentation modes: " + (err));
        }
        ...
        memFree(pSwapChain);
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to create swap chain: " + (err));
        }
        ...
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to get number of swapchain images: " + (err));
        }
        ...
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to get swapchain images: " + (err));
        }
        memFree(pImageCount);
        ...
        for (int i = 0; i < imageCount; i++) {
            ...
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to create image view: " + (err));
            }
        }
    ...
    private static long createClearRenderPass(VkDevice device, int colorFormat) {
        ...
        attachments.free();
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to create clear render pass: " + (err));
        }
        return renderPass;
    }
    ...
    private static long[] createFramebuffers(VkDevice device, Swapchain swapchain, long renderPass, int width, int height) {
        ...
        for (int i = 0; i < swapchain.images.length; i++) {
            ...
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to create framebuffer: " + (err));
            }
    ...
    private static void submitCommandBuffer(VkQueue queue, VkCommandBuffer commandBuffer) {
        ...
        submitInfo.free();
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to submit command buffer: " + (err));
        }
    }
    ...
    private static VkCommandBuffer[] createRenderCommandBuffers(VkDevice device, long commandPool, long[] framebuffers, long renderPass, int width, int height) {
        ...
        int err = vkAllocateCommandBuffers(device, cmdBufAllocateInfo, pCommandBuffer);
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to allocate render command buffer: " + (err));
        }
        ...
        for (int i = 0; i < renderCommandBuffers.length; ++i) {
            ...
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to begin render command buffer: " + (err));
            }
    ...
    private static void submitPostPresentBarrier(long image, VkCommandBuffer commandBuffer, VkQueue queue) {
        ...
        cmdBufInfo.free();
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to begin command buffer: " + (err));
        }
        ...
        if (err != VK_SUCCESS) {
            throw new AssertionError("Failed to wait for idle queue: " + (err));
        }
    ...
    public static void main(String[] args) {
        ...
        final class SwapchainRecreator {
            ...
            void recreate() {
                ...
                cmdBufInfo.free();
                if (err != VK_SUCCESS) {
                    throw new AssertionError("Failed to begin setup command buffer: " + (err));
                }
                ...
                if (err != VK_SUCCESS) {
                    throw new AssertionError("Failed to end setup command buffer: " + (err));
                }
        ...
        GLFWWindowSizeCallback windowSizeCallback = new GLFWWindowSizeCallback() {
            public void invoke(long window, int width, int height) {
                if (width <= 0 || height <= 0)
                    return;
                VulkanUtil.width = width;
                VulkanUtil.height = height;
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to create image acquired semaphore: " + (err));
            }

            // Create a semaphore to wait for the render to complete, before presenting
            err = vkCreateSemaphore(device, semaphoreCreateInfo, null, pRenderCompleteSemaphore);
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to create render complete semaphore: " + (err));
            }

            // Get next image from the swap chain (back/front buffer).
            // This will setup the imageAquiredSemaphore to be signalled when the operation is complete
            err = vkAcquireNextImageKHR(device, swapchain.swapchainHandle, 0xFFFFFFFFFFFFFFFFL, pImageAcquiredSemaphore.get(0), VK_NULL_HANDLE, pImageIndex);
            currentBuffer = pImageIndex.get(0);
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to acquire next swapchain image: " + (err));
            }
            ...
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to submit render queue: " + (err));
            }
            ...
            if (err != VK_SUCCESS) {
                throw new AssertionError("Failed to present the swapchain image: " + (err));
            }
