...
public class StObject extends StPrimitive implements Iterable<Map.Entry<String, StPrimitive>> {
    ...
    public boolean equals(StPrimitive primitive) {
        if(primitive instanceof StObject) {
            StObject other = (StObject) primitive;

            if(other.members.size() != members.size())
                return false;

            for(Map.Entry<String, StPrimitive> member : this) {
                StPrimitive otherPrimitive = other.get(member.getKey());
                if(otherPrimitive == null || !otherPrimitive.equals(member.getValue()))
                    return false;
            }
            return true;
        }
