...
public class StArray extends StPrimitive implements Iterable<StPrimitive> {
    ...
    protected int getDataSize() {
        int size = 2; // short size (index Count prefix)
    ...
    public boolean equals(StPrimitive primitive) {
        if(primitive instanceof StArray) {
            StArray other = (StArray) primitive;

            if(other.length() != length())
                return false;
            for(int i = 0; i < length(); i++) {
                StPrimitive a = other.get(i);
                StPrimitive b = this.get(i);
