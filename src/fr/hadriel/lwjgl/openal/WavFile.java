...
// Finally, set the IO State
    ...
    public static WavFile readWavStream(InputStream inputStream) throws IOException {
...
// Check that the file size matches the number of bytes listed in header
        if (fileLength != chunkSize + 8) {
            throw new IOException("Header chunk size (" + chunkSize + ") does not match file size (" + fileLength + ")");
        }
...
// --------------------------
    ...
    // Short
// ----mono:
    public int readFramesChanel(short[] sampleBuffer, int numFramesToRead, int channel) throws IOException {
        return readFramesChanel(sampleBuffer, 0, numFramesToRead, channel);
    }

    public int readFramesChanel(short[] sampleBuffer, int offset, int numFramesToRead, int channel) throws IOException {
    ...
    // ----
    public int readFrames(short[] sampleBuffer, int numFramesToRead) throws IOException {
        return readFrames(sampleBuffer, 0, numFramesToRead);
    }

    public int readFrames(short[] sampleBuffer, int offset, int numFramesToRead) throws IOException {
        if (ioState != WavFile.IOState.READING) throw new IOException("Cannot read from WavFile instance");
    ...
    public int writeFrames(short[] sampleBuffer, int numFramesToWrite) throws IOException {
        return writeFrames(sampleBuffer, 0, numFramesToWrite);
    }

    public int writeFrames(short[] sampleBuffer, int offset, int numFramesToWrite) throws IOException {
